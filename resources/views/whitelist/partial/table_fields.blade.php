@foreach ( $social->whitelist as $whitelist )
  ...
  <td class="mdl-data-table__cell--non-numeric" >
	<div class="left">		
  	  <img  id="{{$social->id}}"  src="{{$social->image}}" alt="{{$whitelist->social_name}}"  class="user-avatar-twitt-list">
	  </div>  
     ...
     <div class="left">		
  	...
  	<div class="name-twitter">		
  		...
  		@if ($whitelist->location)
  			 <p class="no-margin fs-12" >
  			<i style="width:10px!important"class="material-icons mdl-list__item-icon fs-12 no-margin">location_on</i>{{$whitelist->location}}</p>
  		</div>
  		@endif	
